  ...
  return `
    <div class="card" onclick="location.href='imovel.html?codigo=${imovel.codigo}'">
      ...
      <div class="card-info">
        ...
        <p>${dados.descricao}</p>
        <p><strong>Aluguel: ${
          (dados.valor_aluguel || 0).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })
        }</strong></p>
      </div>
    </div>
  `;
...
fetch("imoveis.json") // se o aluguel.html está dentro de /src, mantenha "imoveis.json"
  .then(res => res.json())
  .then(data => {
    ...
    document.getElementById("filtro-aluguel").addEventListener("change", e => {
      ...
      if (e.target.value === "preco") {
        ...
        ordenado.sort((a, b) => (a.dados_publicos.quartos || 0) - (b.dados_publicos.quartos || 0));
      } else if (e.target.value === "area") {
        ordenado.sort((a, b) => (a.dados_publicos.area_util || 0) - (b.dados_publicos.area_util || 0));
      }
